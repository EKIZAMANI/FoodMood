...
class MainViewModel: ViewModel() {
    ...
    private val _bookmarkPostMsg = MutableLiveData<String>()
    val bookmarkPostMsg: LiveData<String> = _bookmarkPostMsg
    ...
    fun postBookmark(token: String, foodId: Int) {
        var value = ""

        viewModelScope.launch {
            try {
                val apiService = ApiConfig.getApiService(token)
                val response = apiService.postBookmark(BookmarkRequest(foodId))
                _bookmarkPostMsg.value = response.message!!
            } catch (e: HttpException) {
                e.printStackTrace()
            }
        }
